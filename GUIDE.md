...
This document explains the project's findings for readers who want to understand what the Voynich Manuscript encodes without navigating 1,405 constraint files. Everything here is grounded in statistical evidence from the transcript data.
...
The researcher hasn't "translated" anything in the traditional sense — no one handed them a dictionary. But they can prove, from internal structure alone, that the notation encodes music — because the structural constraints fit the physics of sound and no other domain. And once that mapping is established, they *can* tell you what the music sounds like. The structure *is* the semantics.
...
- **MIDDLE** is the primary discriminative content — the specific action variant within the channel the prefix opened. MIDDLEs are themselves compositional, decomposing as **HEAD + MOD\* + TERM** (C1393-C1394):
  ...
  - **MOD stack** (p, c, i, f, d, s) parametrizes the action. The first modifier carries the most weight (66.5% decisive). Each modifier has a consistent category-shifting effect — `d` pulls toward OPERATION (V=0.657), `f` toward MARKING (76.4%), `i` toward TRANSITION (44.7%). Co-occurrence avoidance dominates stacking: 8 of 15 modifier pairs never appear together, driven by HEAD selectivity conflict (C1472-C1473). s is the universal connector; p is reliably early; no strict linear ordering exists.
  - **TERM** (y, l, r, h, m, n) sets the exit condition — r=respond (99% FLOW), y=end (56% OPERATION), h=watch (transparent — lets HEAD+MODS determine category at V=0.988). Terminals follow a three-tier opacity gradient controlling whether information flows to the suffix layer: OPAQUE (m, n, y — suffix rate 0.8–4.2%, token closes without suffix), SEMI-TRANSPARENT (l, r — 17–20% suffix), TRANSPARENT (h — 98.7% suffix, passes through to suffix for complementary information). This is an active grammar rule, not a population effect — opaque terminals suppress suffix even in matched populations (O/E 0.105–0.168; C1440-C1441). The TERMINAL and suffix layers carry complementary information (only 8.2% redundancy; C1442), and at paragraph level they trade off as partial substitutes (rho=-0.199; C1445).
...
- **MOD stack** parametrizes the action. The modifier closest to HEAD carries the most category weight (66.5% decisive). Each modifier has a consistent effect — `d` pulls toward OPERATION, `f` and `p` toward MARKING, `i` toward TRANSITION. 8 of 15 modifier pairs never co-occur due to HEAD selectivity conflict — narrow modifiers (d→e-HEAD 85%, i→a-HEAD 89%, p→o-HEAD 79%) demand incompatible domains. s bridges all frames as the universal connector (C1472-C1474).
...
|-------|--------|--------|--------------|----------|---------|
| qokeedy | qo | keedy | k(heat) + ee(stabilize×2) + d(mark) + y(end) | THERMAL | energy channel: gentle sustained heat, mark completion |
...
This project was built using AI-assisted computational analysis over 541 research phases. The primary development environment was [Claude Code](https://claude.ai/claude-code) (Anthropic), with independent cross-validation from GPT-5 (OpenAI) at key decision points.
...
This matters because no single analytical session — human or AI — could discover 49 instruction classes, 17 forbidden transitions, 6 macro states, 8 cross-system operational categories, an 18-atom instruction encoding architecture, and the Brunschwig alignment in one pass. But 548 phases, each building on validated prior work and never losing what was already proven, could. The constraint system is the project's memory, and its growth is what made the depth of analysis possible.
