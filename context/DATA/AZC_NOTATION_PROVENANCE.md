...
6. Do **not** compute order / transition / self-transition / "rigidity" statistics on R/S/C placement *sequences* and read them as manuscript grammar — placements serialize as depth-sorted blocks, so those statistics are forced (§10; this retracted C434, C433, and C436's self-transition half). Order-independent statistics (vocabulary overlap, etc.) are fine.
7. Do **not** read `line_initial`/`line_final` "edge" rates as a spatial boundary/interior grammar — they are **position indices within a single-placement locus**, so a code's line-edge rate = **1/(mean locus length)** by identity (zero degrees of freedom). High edge-rate just means short loci (labels, `@Lz`); low means long loci (ring text, `@Cc`). This retracted **C435**. The labels-vs-ring-text distinction is the locus subtype (§3), not a positional grammar.
