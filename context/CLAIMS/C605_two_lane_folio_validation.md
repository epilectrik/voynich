...
> The two-lane model (CHSH-lane vs QO-lane) validated at the folio level. Lane balance (= en_chsh_proportion * cc_daiin_fraction vs en_qo_proportion * cc_old_fraction) predicts escape density better than ch_preference alone (|rho|=0.506 vs |rho|=0.301). Lane balance adds independent information beyond ch_preference (partial rho=-0.452, p<0.0001). Lane balance also correlates positively with ch_preference (rho=0.355, p=0.001). The two-lane model captures folio-level execution profiles better than any single PREFIX-based metric.
...
|-----------|--------|-----|---|
| Lane balance | escape_density | **-0.506** | <0.0001 |
| ch_preference | escape_density | -0.301 | 0.006 |
| Lane balance | ch_preference | **0.355** | 0.001 |

Lane balance is 68% stronger than ch_preference at predicting escape density (|rho|=0.506 vs |rho|=0.301).

### Independence Test

Partial correlation: escape ~ lane_balance | ch_preference: rho=-0.452, p<0.0001. Lane balance captures substantial escape variation that ch_preference alone cannot explain.

### Correlation Matrix (Key Pairs)

| Pair | rho | p | Bonferroni |
|------|-----|---|------------|
| escape ~ en_qo_proportion | **0.726** | <0.0001 | ** |
| escape ~ en_chsh_proportion | **-0.704** | <0.0001 | ** |
| escape ~ cc_daiin_fraction | **-0.431** | 0.0001 | ** |
...
The correlation structure confirms the two-lane model: EN subfamily and CC composition are tightly coupled, and their interaction predicts escape behavior.
...
1. **CHSH lane:** Activated by CC_DAIIN, produces ch/sh-prefixed EN tokens, participates in hazards
2. **QO lane:** Activated by CC_OL_D, produces qo-prefixed EN tokens, associated with escape routes
