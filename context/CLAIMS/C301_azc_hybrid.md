# C301: AZC is HYBRID

**Tier:** 2 | **Status:** CLOSED | **Phase:** AZC

---

## Claim

The Astronomical/Zodiac/Cosmological (AZC) text is a genuine HYBRID system that bridges Currier A and B, belonging fully to neither.

## Classification Evidence

| Metric | AZC | Threshold | Verdict |
|--------|-----|-----------|---------|
| B vocabulary coverage | 69.7% | 70% | Just below |
| A vocabulary coverage | 65.4% | - | Moderate |
| Shared vocabulary (A∩B) | 60.5% | - | Core overlap |
| Unique vocabulary | 25.4% | - | Distinct terms |

## Distinctive Properties

| Property | AZC | Currier A | Currier B |
|----------|-----|-----------|-----------|
| Token percentage | 7.7% | 30.5% | 61.9% |
| Folios | 30 | 114 | 83 |
| TTR | 0.285 | 0.137 | 0.096 |
| Tokens/line (median) | 8 | 22 | 31 |
| LINK density | 7.6% | 3.0% | 6.6% |

## AZC-Unique Vocabulary (C304-305)

- 1,529 types (25.4%) absent from both A and B
- 98% section-exclusive
- 37% line-initial, 37% line-final
- 65.9% hapax (most appear only once)
- Structural signature: LABELING function

## Related Constraints

- C300 - 9,401 tokens unclassified by Currier
- C302 - Distinct line structure
- C303 - Elevated LINK density
- C306 - Placement-coding axis
- C322 - SEASON-GATED WORKFLOW interpretation

---

## Navigation

← [INDEX.md](INDEX.md) | ↑ [../CLAUDE_INDEX.md](../CLAUDE_INDEX.md)
