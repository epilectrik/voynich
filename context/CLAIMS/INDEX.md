...
| 1957 | Suffix boundary revision: blocked e-initial and h-initial suffixes from extract() parser. e=cool is a MOD atom encoding e_depth (C1225); h=watch is a MIDDLE terminal (C1487). Both steal operational atoms from MIDDLE when parsed as suffix-initial. Fix: e_depth match MIDDLE-vs-atoms rises from 16.7% to 98.6%; max folio distortion drops from 14pp to 0.000. Suffix set reduced to d/a/i/o-initial + single TERM atoms. Zero empty-MIDDLE regression | 2 | B, morphology, SUFFIX, atom, C1225, C1487, C1511 | e_match_before=16.7%. e_match_after=98.6%. max_distortion=0.000. tokens_fixed=3000+. |
| 1958 | ot PREFIX = transfer rate / drip rate monitoring (upgraded from "operational verification"). ok/ot ratio correlates with recipe emphasis: f83r (Ch9P drip-counting recipe) has lowest ok/ot=0.38; f81v (Ch18M potable gold/maceration) has highest ok/ot=5.50. On f83r L22, ot×3 clusters exactly where Catalan text specifies drop counting at 6/10/15/20. Three ot MIDDLEs encode check(otchedy), iterate(otaiin), done(otedy) — control actions around drip monitoring, not numerical values. Control loop: sh→qo→ok→ot→sh = watch→heat→vessel temp→drip rate→watch | 2 | B, PREFIX, ot, ok, C929, C1313, C1316 | f83r_ok_ot=0.38. f81v_ok_ot=5.50. L22_ot=3. catalan_validated=yes. |
...
| C# | Claim | Tier | Scope | Key Metrics |
|----|-------|------|-------|-------------|
| 1891 | f76r P1 has the strongest monotonic monitoring gradient in Currier B (rank 1/13 paragraphs with 15+ lines, Spearman rho=0.710). Not part of 8D matching features (which use folio-level HEAD ratios, not within-paragraph positional gradients). Small comparison set (n=13) | 2 | B, PREFIX, monitoring, gradient | rank=1of13. rho=0.710. min_lines=15. n_eligible=13. |
| 1892 | f76r P1 monitoring gradient loads more on ch (rho_ch=0.341) than sh (rho_sh=0.221). Consistent with Ch18's active testing procedure (C929: ch=active test). Difference not formally tested for significance at this sample size | 2 | B, PREFIX, monitoring, C929 | rho_ch=0.341. rho_sh=0.221. difference=0.120. |
| 1893 | f75r and f76r show PREFIX balance shift within same section/REGIME/quire: f75r qo-enriched (26.2% vs f76r 19.1%), f76r ch-enriched (17.0% vs f75r 10.2%). Both folios in section B, REGIME_1, quire M — section effects controlled. Consistent with C929/C1313 k/e channel architecture. 8D matching features use HEAD ratios not PREFIX fractions (not circular) | 2 | B, PREFIX, k-channel, e-channel, C929, C1313 | f75r_qo=0.262. f76r_qo=0.191. f75r_ch=0.102. f76r_ch=0.170. same_section=B. same_regime=R1. |
...
| 1895 | Blind prediction test: formalized predictions (tertile thresholds) scored against all 82 B folios. Null rate 42.3%, matched rate 61.3%, lift 1.45x. Ch24->f84v significant (p=0.024, rank 1/82). Ch9->f83r and Ch16->f108r suggestive (p=0.171). Ch27->f77v and Ch18t->f81v worse than random (p>0.84). Aggregate lift is modest; individual chapter results vary widely | 3 | B, matching, PL, methodology | null_rate=0.423. matched_rate=0.613. lift=1.45. Ch24_p=0.024. sig_at_020=3of7. |
| 1896 | C1884 upgraded from Tier 4 to Tier 3 for Ch19->f75r and Ch18->f76r based on structural convergence (C1891-C1894 + C1889). Supporting evidence has caveats: C1891 small n, C1892 untested difference. Ch12->f113v remains Tier 4 | 3 | B, matching, PL, content, upgrade | upgraded=Ch19_f75r+Ch18_f76r. remaining_T4=Ch12_f113v. |

**Phase 629 findings (Crib Decode Validation, CONTENT_VALIDATED):**
- Two crib decodes produce structural evidence beyond Phase 628's 8D matching features. f76r P1's monitoring gradient ranks #1 of 13 eligible paragraphs (C1891, rho=0.710, small comparison set). The gradient loads more on ch than sh (C1892), consistent with Ch18's active testing, though the ch-sh difference is not formally tested.
- PREFIX balance shift between f75r and f76r (C1893): f75r is qo-enriched while f76r is ch-enriched, within the same section/REGIME/quire (controlling for section effects). The 8D matching features use HEAD ratios, not PREFIX fractions, so this is not circular with the matching algorithm. Consistent with C929/C1313 k/e channel architecture.
...
- Blind prediction null baseline (C1895): formalized predictions scored against all 82 folios give null rate 42.3%, matched rate 61.3%, lift 1.45x. Aggregate lift is modest, but individual results vary: Ch24→f84v is significant (p=0.024, rank 1/82), while Ch27→f77v and Ch18t→f81v perform worse than random (p>0.84), independently confirming they are the weakest matches.
- Combined evidence upgrades C1884 from Tier 4 to Tier 3 for Ch19→f75r and Ch18→f76r (C1896), with caveats noted on supporting constraints.
...
### Phase 625: Short Paragraph Architecture — SECTION_CONFOUNDED_WITH_LENGTH_RESIDUAL (C1848-C1858)

| C# | Claim | Tier | Scope | Key Metrics |
|----|-------|------|-------|-------------|
| 1848 | Short paragraphs (0-4 body lines, 85.8% of B) NOT truncated beginnings of long paragraphs: position-matched subsample null fails at pooled level (4/11 features pass KS p>0.05, need 8+). Both position-matched and random subsamples fail similarly — pooled distinctness is driven by section composition mismatch, not positional truncation | 2 | B, paragraph, stratum, subsample, C1206 | n_pass_pos=4. n_pass_rand=4. verdict=DISTINCT. n_minimal=183. n_short=143. n_long=112. n_header_only=90. |
| 1849 | Section dominates stratum variance: section x stratum V=0.468 (p<0.001). After section residualization, 8/11 paragraph features become stratum-invariant (SHORT vs LONG). Within Recipe: 10/11 invariant. Golden folio (48 folios with both strata from same section): 7/11 invariant. Most apparent stratum effects are section selection artifacts | 2 | B, paragraph, section, stratum, C860, C1404 | V=0.468. resid_invariant=8of11. recipe_invariant=10of11. golden_invariant=7of11. n_golden=48. |
| 1850 | Three features show genuine stratum effects after section control: cat_entropy (p=3.2e-5, d=-0.54 — operational diversity narrows mechanically with fewer lines), tokens_per_line (p=0.038, d=0.34 — short paragraphs pack more per line), m_terminal_rate (p=0.000248, d=-0.25 — closure marker frequency). All length-mechanical, not operational mode differences | 2 | B, paragraph, stratum, features, length_residual | cat_entropy_p=3.2e-5. cat_entropy_d=-0.54. tokens_p=0.038. tokens_d=0.34. m_term_p=0.000248. m_term_d=-0.25. |
| 1851 | Zone classification (C1398) stratum-independent: V=0.098 (p=0.206) across MINIMAL/SHORT/LONG. MINIMAL paragraphs span all operational zones uniformly (OPERATION 56%, THERMAL_QO 29%, CONTAINMENT 13%, MONITORING 2%). Within Recipe: V=0.101, p=0.447. Short paragraphs are compressed general-purpose units, not zone-specialized | 2 | B, paragraph, zones, stratum, C1398 | V=0.098. p=0.206. recipe_V=0.101. recipe_p=0.447. |
| 1852 | HEADER_ONLY paragraphs overwhelmingly non-gallows-initial: 87.8% (79/90) have no gallows-initial token. Opener fraction (k+f) = 4.4% (lowest of all strata vs LONG 8.9%). k-fraction enriched in header atoms (0.157 vs 0.116, p=0.049), o-fraction depleted (0.096 vs 0.142, p=0.0002). Reverses C1780/C1784 prediction — HEADER_ONLY are NOT executive gallows declarations | 2 | B, paragraph, header_only, gallows, C1780, C1784 | opener_ho=0.044. opener_long=0.089. none_frac=0.878. k_frac_ho=0.157. o_frac_ho=0.096. k_p=0.049. o_p=0.0002. |
| 1853 | HEADER_ONLY do not create paragraph dependencies after section control: post-HO paragraphs differ in 8/11 features pooled (section confound) but 0/11 within Recipe (2 post-HO paragraphs). Punctuation function borderline: zone change rate 64.7% vs baseline 47.7%, permutation p=0.06 (not significant). Consistent with C845 self-containment | 2 | B, paragraph, header_only, independence, C845, C1399 | pooled_sig=8of11. recipe_sig=0of11. zone_change=0.647. baseline=0.477. perm_p=0.06. |
| 1854 | No anti-parallel arc in category-level feature space: first/last body line cosine=+0.999 at all strata (MINIMAL 0.9992, SHORT 0.9992, LONG 0.9993). C1837's anti-parallel finding (cosine=-0.989) operates at atom enrichment grain not captured by 12 category-level features. Caveat: features not z-scored, tokens_per_line (d>2.0 first vs last) dominates cosine | 2 | B, paragraph, arc, boundary, C1837, C1842 | cos_minimal=0.9992. cos_short=0.9992. cos_long=0.9993. tpl_d_minimal=2.52. tpl_d_long=1.67. |
| 1855 | Kernel h-rate declining gradient (C1206) not detected at paragraph body-position level in any stratum. SHORT: rho=-0.078, p=0.086. LONG: rho=-0.035, p=0.278. MINIMAL: rho=-0.041, p=0.539. C1206 gradient may operate at finer position resolution, within specific sections, or at sub-paragraph scale not captured by between-body-line analysis | 2 | B, paragraph, kernel, gradient, C1206 | short_rho=-0.078. short_p=0.086. long_rho=-0.035. long_p=0.278. |
| 1856 | Header-body feature coupling increases monotonically with paragraph length: MINIMAL mean|rho|=0.070, SHORT 0.078, LONG 0.098. Longer paragraphs allow more header→body prediction. Consistent with C1795 (dR2=+0.063). Coupling does NOT increase with shortness — short paragraphs are more header-independent | 2 | B, paragraph, header, coupling, C1795 | minimal_rho=0.070. short_rho=0.078. long_rho=0.098. increases_with_shortness=false. |
| 1857 | HIGH-count folios (9+ paragraphs) show stronger sequential organization: steeper first-line length gradient (rho=-0.247, p=0.033 vs LOW rho=+0.261), more structured gallows transitions (chi2=37.8, p=1.9e-5, V=0.236), MORE paragraph diversity (mean within-folio JSD=0.146 vs LOW 0.119). HIGH-count folios are internally heterogeneous recipe collections with strong ordering | 2 | B, folio, paragraph, organization, gradient | high_rho=-0.247. high_p=0.033. high_chi2=37.8. high_V=0.236. high_jsd=0.146. low_jsd=0.119. |
| 1858 | Section-specific truncation: within Recipe, MINIMAL paragraphs pass position-matched subsample null (8/11 features, n_real=163, n_synth=44). Recipe MINIMAL specifically resembles truncated beginnings of Recipe LONG. B-section (9/11, n_real=5) and H-section (9/11, n_real=12) show same pattern. Pooled failure (C1848) is driven by cross-section composition mismatch | 2 | B, paragraph, section, Recipe, truncation, C1848 | S_n_pass=8. S_n_real=163. B_n_pass=9. B_n_real=5. H_n_pass=9. H_n_real=12. |

**Phase 625 findings (Short Paragraph Architecture, SECTION_CONFOUNDED_WITH_LENGTH_RESIDUAL):**
- Short paragraphs (0-4 body lines, 85.8% of B) are NOT operationally distinct from long paragraphs. The apparent stratum differences are overwhelmingly section selection artifacts: Recipe dominates MINIMAL (89%), Herbal/Bio dominate LONG (80%). After section control, 8-10 of 11 features become invariant.
- Three features survive section control (C1850): cat_entropy, tokens_per_line, m_terminal_rate. All are length-mechanical — shorter text mechanically reduces category diversity, alters line-packing density, and shifts closure marker frequency. These are NOT evidence of different operational modes.
- Within-section, MINIMAL paragraphs PASS the position-matched subsample null (C1858, 8/11 in Recipe). The pooled DISTINCT verdict (C1848) is explained by cross-section composition mismatch, not genuine structural distinctness. Short paragraphs within a section ARE truncated beginnings of that section's long paragraphs.
- Zone classification is stratum-independent (C1851): MINIMAL/SHORT/LONG all distribute across C1398 zones identically. Short paragraphs are compressed general-purpose units, not zone-specialized.
- HEADER_ONLY paragraphs are 87.8% non-gallows-initial (C1852) — they are NOT executive declarations opened by gallows. They show k-enrichment and o-depletion in header atoms but do not create paragraph dependencies (C1853, 0/11 significant within Recipe).
- No anti-parallel arc structure in category-level features at any scale (C1854, cosine=+0.999). C1837's result operates at finer atom-enrichment grain.
- The kernel h-rate gradient (C1206) is not detected at body-position level in any stratum (C1855), including LONG — this specific test granularity may not capture the gradient.
...
### Phase 605: Procedure-Family Contrast Alignment — PARTIAL_FAMILY_CONTRAST (C1752-C1754)

| C# | Claim | Tier | Scope | Key Metrics |
|----|-------|------|-------|-------------|
| 1752 | Thermal axis alignment confirmed: h_ratio_resid (residualized on section + k_ratio) negatively correlates with thermo_ke (Spearman rho=-0.400, p=0.0003, n=69) and thermal paragraph fraction (rho=-0.473, p=0.0009, n=41). Pseudo-Lull distillation is heat-dominant (heat_rate 13.33 vs sublimation 11.37); V folios with higher monitoring residual have lower thermal burden. Survives N1 permutation (frac_exceeding=0.002), N2 random axis (0/4 pass), N3 wrong-direction (p>0.99), within-Herbal replication (rho=-0.570, p=0.001, n=27), and section+REGIME control (rho=-0.391/-0.460). First clean historical family contrast recovered inside Voynich | 2 | B, pseudo-Lull, thermal, monitoring, h_ratio, distillation, sublimation, C1750, C1735, C1744 | P3_rho=-0.400. P3_p=0.0003. P4_rho=-0.473. P4_p=0.0009. N1_frac=0.002. N2_passes=0. herbal_P3_rho=-0.570. herbal_P3_p=0.001. |
| 1753 | Termination/iteration family contrasts directionally correct but sub-threshold after residualization: P1 terminal_rate rho=+0.158 (p=0.098, n=69), P2 iteration_rate rho=+0.169 (p=0.083, n=69). PL sublimation has 2.5x termination and 2.4x chain_rate vs distillation. Both predictions in correct direction but fail p<0.05. Raw h_ratio recovers P1 (rho=+0.377, p=0.0007) — section confound partially mediates. V closure and chaining may be too line-local (C1471) for folio-level family contrast recovery | 2 | B, pseudo-Lull, termination, iteration, monitoring, C1471, C1746, C1398 | P1_rho=0.158. P1_p=0.098. P2_rho=0.169. P2_p=0.083. P1_raw_rho=0.377. P1_raw_p=0.0007. |
| 1754 | Family contrast is distillation-vs-rest, not sublimation-specific: N4 dissolution contrast shows P3/P4 thermal predictions match dissolution (dissolution heat_rate 7.09 < distillation 13.33, same direction as sublimation 11.37). Both sublimation and dissolution are "less thermal than distillation." P1/P2 directions diverge (dissolution termination 2.59 < distillation 4.87, opposite to sublimation 12.20 > distillation). Recovered signal is a distillation-centered thermal family vs monitoring-rich non-distillation basin, not unique sublimation identification | 2 | B, pseudo-Lull, dissolution, distillation, sublimation, thermal, families, C1750, C1752 | N4_P3_match=true. N4_P4_match=true. N4_P1_diverge=true. N4_P2_diverge=true. dissolution_heat=7.09. distillation_heat=13.33. sublimation_heat=11.37. |

**Phase 605 findings (Procedure-Family Contrast Alignment, PARTIAL_FAMILY_CONTRAST):**
- Pre-registered verdict PARTIAL_FAMILY_CONTRAST (K_perm=2, N2 passes). Two of four primary predictions pass with strong effect sizes, both on the thermal axis. The monitoring-residual (h_ratio_resid) negatively correlates with thermal burden, exactly as predicted from PL distillation-sublimation heat_rate contrast.
- Thermal axis is the first clean historical family contrast recovered inside Voynich (C1752): pseudo-Lull's distillation vs sublimation heat_rate ratio (0.85x) predicts that V folios with higher monitoring residual have lower thermal content. This survives every control: permutation, random axis, wrong-direction, within-Herbal (where it strengthens to rho=-0.57), and REGIME control. The bridge is real.
- Termination and iteration mappings are directionally correct but underpowered (C1753): P1/P2 show rho~0.16 in predicted direction but miss the p<0.05 threshold. Raw h_ratio recovers P1 (rho=0.38, p=0.0007), indicating section-mediated component. V closure and chaining are line-local phenomena (C1471) that may not aggregate cleanly to folio-level family contrasts.
- Distillation is the recoverable family anchor, not sublimation (C1754): N4 dissolution contrast shows dissolution would make the same thermal predictions as sublimation (both less thermal than distillation). The recovered signal is distillation-vs-rest, not sublimation-specific. Historically coherent: distillation is the thermally dominant family across Brunschwig (C1735), pseudo-Lull, and now Voynich.
- Expert synthesis: the Voynich and pseudo-Lull inhabit the same procedure-space, at least at the level of a distillation thermal baseline versus more monitoring-intensive alternative procedures. Not a full family taxonomy, but a historically meaningful operational contrast axis, now triangulated by Brunschwig, pseudo-Lull, and Voynich internal manifold.
...
| C# | Claim | Tier | Scope | Key Metrics |
|----|-------|------|-------|-------------|
| 1739 | Closure-response alignment not confirmed: historical closure-burden features (3 axes: containment density, intervention density, recycle complexity) from 431 Brunschwig 1512 recipes do not predict Voynich 7D closure-response phenotype across section×REGIME cells. 0/4 tests: Mantel r=-0.385 (p=0.866), Stars 3/4 concordant but combined p=0.319, rank 0/3, Herbal 0/3. Annotation stable (H1 rho=0.744, H2 rho=0.842). Failure is structural: bridge assumes monotone containment→preventive safety mapping, but Voynich closure manifold is profile- and threshold-dependent (C1639-C1648, C1732-C1733). H:R2 reversal (lowest ey, highest ii) consistent with authenticity-sensitive containment loading onto transformative safety. Individual safety axes within Stars significant (see C1740) | 2 | B, Stars, apparatus, Brunschwig, REGIME, closure, safety, C1735, C1737, C1639, C1642 | P1_mantel_r=-0.385. P1_p=0.866. P2_concordant=3/4. P2_combined_p=0.319. P3_concordant=0/3. P4_concordant=0/3. H1_rho=0.744. H2_rho=0.842. prediction_hash=eb6824e8. |
...
- Cross-section closure-demand bridge fails structurally (C1739): 0/4 pre-registered tests pass. The bridge assumed a monotone mapping from containment burden to closure-response phenotype, but the Voynich closure manifold is profile- and threshold-dependent. H:R2 (SEALED_RECIRCULATION) comprehensively reverses: lowest ey_rate (0.0604), highest ii_rate (0.1136) — consistent with authenticity-sensitive containment regimes loading onto transformative rather than preventive safety (C1639-C1647, C1733). Sealedness ≠ preventive safety discipline.
- Stars safety substitution confirmed at individual axis level (C1740): ey_rate replicates C1735 (p=0.0003), ii_rate provides NEW confirmation of safety substitution (p=0.0259). DYE_advantage reverses — DYE behaves as an intervention-productivity metric (C1633-C1634), orthogonal to the preventive/transformative safety axis, not a generic closure-quality scalar.
- At present, the only replicated Brunschwig→Voynich alignment is thermal-intensity / safety-substitution structure within Stars (C1735, C1740) and within-folio THERMAL gradient (C1736). Broader cell-level bridges from historical method or closure-demand prototypes remain unconfirmed (C1737, C1739). The Brunschwig alignment is narrower and more section-conditioned than broad multi-cell prototype models assumed.
...
| C# | Claim | Tier | Scope | Key Metrics |
|----|-------|------|-------|-------------|
| 1737 | Apparatus bundle alignment not confirmed for the tested Brunschwig→secondary-profile bridge family. 0/4 tests pass: Mantel geometry anti-correlated (median r=-0.279, p=0.794), dominant match 0.25 (all cells SEALED_VESSEL dominated), Stars R1-R3 direction 1/3 concordant (p=0.913), open-cycle cosine=-0.606 (p=0.806). Robustly negative across 48 bridge variants (0% significant). DISTILLATION diagnostic clean (p=0.624). Result is failure of the tested bridge family, not disconfirmation of Voynich apparatus structure itself (which remains validated by C1248, C1380, C1625-C1629, C1640, C1668, C1722). P4 design concern: distill_references>=2 may not correspond to open-cycle/unseal intervention (C1247) | 2 | B, apparatus, Brunschwig, REGIME, secondary profiles, C1248, C1735, C1736 | P1_mantel_r=-0.279. P1_p=0.794. P2_match=0.25. P3_concordant=1/3. P4_cosine=-0.606. bridge_variants=48. pct_significant=0%. prediction_hash=5dded97c. |
| 1738 | SEALED_VESSEL universal secondary dominance at cell-mean level: all 5 viable section×REGIME cells show SEALED_VESSEL as dominant secondary profile (range 0.328-0.609). S:R1=0.609, S:R3=0.516, H:R2=0.488, H:R4=0.374, H:R3=0.328. 4/5 non-ambiguous (margin>0.02). Compressed expected dominant-axis contrast under the tested historical bridge. Cells may still differ meaningfully in relative proportions, geometry, and distributional shape while sharing SEALED_VESSEL top-1. SV vocabulary may encode general containment/waiting operations rather than apparatus-specific sealed-vessel indicators | 2 | B, apparatus, SEALED_VESSEL, secondary profiles, C1248, C1249 | SV_range=0.328-0.609. n_cells=5. non_ambiguous=4/5. |

**Phase 599 findings (Apparatus Bundle Alignment, APPARATUS_ALIGNMENT_NOT_CONFIRMED):**
- No support for the tested bridge family (C1737): Pre-registered 48 bridge variants mapping 4 Brunschwig method-bundle classes to Voynich secondary apparatus profiles. All 4 tests fail robustly (not a single variant achieves significance). DISTILLATION diagnostic clean (p=0.624) — genuine secondary-space failure, not thermal intensity confound. Result interpreted as failure of the tested bridge framework, not disconfirmation of Voynich internal apparatus structure (C1248, C1380, C1625-C1629, C1640, C1668, C1722 remain valid).
- SEALED_VESSEL dominates secondary space at cell-mean level (C1738): Every section×REGIME cell has SEALED_VESSEL as its dominant secondary profile. This compressed the discriminability expected under the tested bridge. However, internal apparatus differentiation may still exist in relative proportions, distributional shape, or covariance structure — sealedness may be a common background operating condition in all control programs.
- P4 design concern: "open-cycle" recipe definition (distill_references >= 2) may be misdefined — captures multi-distillation recipes concentrating gentle sustained methods, not reopening/unseal interventions per C1247. This weakens P4 specifically.
- Thermal intensity alignment (C1735/C1736) remains the strongest confirmed Brunschwig-Voynich alignment. This phase did not recover additional apparatus-shape alignment, but the question remains open under alternative bridge designs (distributional, closure-based, containment-response).
...
| C# | Claim | Tier | Scope | Key Metrics |
|----|-------|------|-------|-------------|
| 1681 | Zodiac category seasonal signal: SEASONAL_SIGNAL_CONFIRMED (after zodiac correction). Standard scholarship zodiac map has ≥6 misassigned folios + 2 non-zodiac pages included. Corrected confident-only map (7 folios): chi2=48.73, V=0.157, perm_p=0.018. Visual-Aries map (9 folios): V=0.138, perm_p=0.033. Signal is real but was masked by incorrect assignments | 2 | AZC, zodiac, category, season | confident_chi2=48.73. confident_V=0.157. confident_perm_p=0.018. aries_perm_p=0.033. |
| 1682 | Thermal seasonal gradient absent: THERMAL_GRADIENT_ABSENT. THERMAL and CONTAINMENT categories not individually significant in any mapping variant. Seasonal signal is distributed across categories, not concentrated in apparatus-specific channels | 2 | AZC, zodiac, category, THERMAL, CONTAINMENT, season, negative | THERMAL_p>0.2. CONTAINMENT_p>0.16. all_variants. |
| 1683 | Within-season coherence trend: COHERENCE_TREND. Approaches significance with corrected map (visual-Aries: p=0.060). Consistent direction: within-season JSD < between-season JSD in all mapping variants | 2 | AZC, zodiac, category, coherence, season | aries_MWU_p=0.060. confident_MWU_p=0.101. direction_consistent=true. |
| 1684 | Goat-folio seasonal identity: GOAT_PAGES_SPRING. Goat pages (f70v1, f71r) behave like Spring pages not Winter. Goat=Capricorn (Winter) kills seasonal signal (perm_p=0.220), goat=Aries (Spring) preserves it (perm_p=0.033). Diagnostic constraint on zodiac assignment | 2 | AZC, zodiac, category, season, goat, identity | capricorn_perm_p=0.220. aries_perm_p=0.033. diagnostic=true. |

**Phase 583 findings (Zodiac Seasonal Category Clustering, SEASONAL_SIGNAL_CONFIRMED/THERMAL_GRADIENT_ABSENT/COHERENCE_TREND/GOAT_PAGES_SPRING):**
- Zodiac correction critical (C1681 SEASONAL_SIGNAL_CONFIRMED): Standard zodiac folio map includes 2 non-zodiac pages (f70r1/f70r2 = non-figurative diagrams) and has ≥6 misassigned signs (visual evidence vs assumed sequential order). Correcting to visual-evidence assignments changes verdict from WEAK (perm_p=0.079) to CONFIRMED (confident-only perm_p=0.018, V=0.157). The 7 confidently-identified pages give the strongest signal, meaning uncertain pages add noise.
- Apparatus-specific prediction fails (C1682): THERMAL and CONTAINMENT categories show no individual significance in any of the 4 mapping variants tested. The seasonal signal is real but distributed across all categories — not concentrated in the apparatus-configuration channels that Brunschwig's zodiac conditionals would predict.
- Coherence trend (C1683 COHERENCE_TREND): Within-season JSD consistently lower than between-season across all variants. Closest to significance in visual-Aries map (p=0.060). With only 7-9 folios in 4 seasons, power is limited; the trend direction is robust.
- Goat diagnostic (C1684 GOAT_PAGES_SPRING): Assigning goat-center folios (f70v1, f71r) to Capricorn (Winter) destroys the seasonal signal (perm_p=0.220), while Aries (Spring) preserves it (perm_p=0.033). This is a structural identity constraint: the goat pages' category profiles cluster with Spring, not Winter. Three folios (f71v, f72r1, f72r3) remain unidentified — candidates for assignment inference in follow-up phase.
...
### Phase 668: Folio Cold Reads Against Matched Recipes — ALL_COHERENT_OR_PLAUSIBLE (C1971-C1976)

All 15 matched folios cold-read against their Pseudo-Lull Testamentum recipes at the paragraph level. 12 coherent, 3 plausible, 0 incoherent. Cross-folio patterns confirmed: e-depth encodes thermal regime, observation MIDDLE density tracks monitoring requirement, paragraph count tracks recipe complexity, dar distribution tracks material-addition pattern. Polyalphabetic cipher hypothesis rejected.

| C# | Claim | Tier | Scope | Key Metrics |
|----|-------|------|-------|-------------|
| 1971 | Cold read coherence: 15/15 matched folios produce coherent or plausible paragraph-level readings against Pseudo-Lull Testamentum recipes (12 coherent, 3 plausible, 0 incoherent). All 3 CONFIRMED-tier matches coherent; all 4 strong-supported coherent; 5/8 supported coherent, 3 plausible (specification/philosophical chapters). Atom glosses (C1394/C1195) produce control-grammar readings that align with recipe operations without post-hoc adjustment. | 2 | B, recipe-correspondence, cold-read, paragraph, atom | coherent=12. plausible=3. incoherent=0. confirmed_coherent=3of3. strong_coherent=4of4. supported_coherent=5of8. |
| 1972 | e-depth quantitative thermal encoding across 15 matched folios. e-depth tracks physical chemistry thermal requirements at paragraph resolution. Range 0.09 (final coagulation fire, f107r P18) to 1.50 (cold collection, f79r P9 / f107r P7). Balneum mariae paragraphs cluster 0.55-0.67; distillation/condensation 0.92-1.20; strongest fire 0.09-0.34. Cross-validates C1967 non-prefix gradient against external recipe content. | 2 | B, atom, e-depth, thermal, recipe-correspondence, C1967, C1735, C1225 | min=0.09 (f107r P18). max=1.50 (f79r P9, f107r P7). balneum_range=0.55-0.67. distillation_range=0.92-1.20. fire_range=0.09-0.34. n_folios=15. |
| 1973 | Observation MIDDLE density tracks recipe monitoring requirement across 15 matched folios. ckh/cth/ecth/ckhh/cthh density correlates with recipe type: fusibility test (f116r ~23) and calcination (f80r 23) produce dense observation; patience/coagulation (f82r 3, f107r 2) produce near-zero. Active intervention paragraphs carry observation MIDDLEs; autonomous processing paragraphs lose them (fade-out confirmed on f79r, f103r, f82r, f76v, f80r). ecth (cooled-transfer-watch) concentrates on calcination/transfer recipes (f80r: 11 tokens). | 2 | B, MIDDLE, observation, recipe-correspondence, paragraph, C929 | f116r_obs=~23. f80r_obs=23. f82r_obs=3. f107r_obs=2. f80r_ecth=11. fade_out_confirmed=5_folios. |
| 1974 | Paragraph count tracks recipe step complexity across 15 matched folios. Range 2 (two-phase potable gold, f81v) to 18 (highly iterative coagulation, f107r). Simple single-operation recipes → 2-4 paragraphs; multi-step distillation/sublimation → 8-12; highly iterative → 14-18. Extends C1399/C1400 paragraph model. | 2 | B, paragraph, recipe-correspondence, C1399, C1400 | min_para=2 (f81v). max_para=18 (f107r). n_folios=15. |
| 1975 | dar distribution tracks recipe material-addition pattern across 15 matched folios. Five patterns: front-loaded (preparation then processing, f103r 81% in P1-P5), back-loaded (material renewal during reiteration, f75r P9 46%), zero-after-midpoint (cohobation recycling, f112r P6-P14 zero dar), extreme-density (specification/dissolution, f77v P2 17.2%, f81v P1 16.5%), uniform-low (existing material, f107r 2.0%). | 2 | B, PREFIX, dar, recipe-correspondence, paragraph | f103r_front=81%. f75r_back=46%. f112r_zero_after_P5. f77v_P2_density=17.2%. f107r_rate=2.0%. n_folios=15. |
| 1976 | Polyalphabetic cipher hypothesis REJECTED. Soft atoms (d,o,c,p,s,f,r) tested for meaning shift across prefix classes via bigram cosine similarity. Core atoms stable: d=0.994, o=0.924, r=0.939. Lower c=0.640, p=0.660, f=0.609 explained by positional artifact (PSEUDO_HEAD under ch vs MOD under qo), not semantic shift. One cipher system; atoms have stable meaning across all prefix channels. | 1 | B, atom, cipher, falsification, C1394 | d_cosine=0.994. o_cosine=0.924. r_cosine=0.939. s_cosine=0.790. c_cosine=0.640. p_cosine=0.660. f_cosine=0.609. |

**Phase 668 findings (Folio Cold Reads, ALL_COHERENT_OR_PLAUSIBLE / POLYALPHABETIC_REJECTED):**
- Headline result (C1971): all 15 matched folios cold-read coherently or plausibly against their Catalan Testamentum recipes. No post-hoc adjustment of atom glosses was required — the C1394 HEAD+MOD*+TERM model and C1195 atom glosses produced readings that track recipe operations at paragraph resolution. The 3 plausible (not coherent) readings are all specification/philosophical chapters where recipe text is too discursive for precise paragraph mapping.
- Thermal encoding (C1972): e-depth is the manuscript's thermometer. The 0.09-1.50 range across 15 folios maps quantitatively to physical chemistry: balneum mariae (water bath) at ~0.6, distillation condensation at ~1.0, and maximal fire at ~0.1-0.3. Cross-validates C1967's statistical gradient (qo>ch>sh, p=0.024) with paragraph-level external recipe content. Key examples: f79r's V-shaped arc (moderate→0.34 strongest fire→1.50 cold collection→0.45 fixation); f112r's oscillation between ashes (0.45) and balneum (0.95) encoding alternating distillation method; f107r's full 0.09-1.50 range spanning the widest thermal envelope.
- Observation vocabulary (C1973): observation MIDDLE density is a recipe-type fingerprint. Fusibility testing (f116r) and calcination (f80r) — both inherently observation-intensive operations — produce ~23 observation MIDDLEs per folio. Patience/coagulation recipes (f82r, f107r) produce 2-3. Within folios, observation MIDDLEs fade out when recipes shift from active intervention to autonomous processing (sublimation, multiplication, cohobation). The hierarchy ckh→cth→ecth→cthh/ckhh maps to heat-check→transfer-watch→cooled-transfer-watch→extended variants.
- Paragraph complexity (C1974): paragraph count monotonically tracks recipe procedural step count from 2 (f81v two-phase) to 18 (f107r iterative coagulation). This extends C1399/C1400: paragraphs are self-contained operational units whose cardinality reflects recipe complexity.
