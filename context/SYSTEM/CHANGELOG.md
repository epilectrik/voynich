...
Phase 601 tests whether A2-like apparatus forgivingness/authenticity regime shifts safety style toward transformative intervention. Pre-registered 8-test design: Stars confirmatory anchor (S2), global forgivingness association (P1), Herbal within-REGIME core test (P2), Herbal A3 surgery (P3), closure authenticity (P4), DYE orthogonality (S1a/S1b), A2 dummy sensitivity (S3). Pre-registered verdict: **STARS_ONLY_REPLICATION** (S2+P3+P4 pass, P1+P2 fail). Substantive verdict: **PROFILE_CONDITIONED_SAFETY_MODERATION**. The pre-registration asked the wrong statistical question (continuous gradient) and got the right structural answer (discrete profile regime). A2 is a compound apparatus regime that categorically shifts safety toward transformative (S3 p=1e-6). Removing A2 rescues Herbal (P3 p=0.021). Closure authenticity modulates safety within sections (P4 rho=0.304, p=0.008).
...
| **ADDED** | C1741: A2 compound apparatus regime categorical safety shift -- A2 dummy coeff=-0.124, t=-5.31, p=1e-6 |
| **ADDED** | C1742: Closure authenticity modulates safety strategy -- strong_close_fraction partial rho=0.304, p=0.008 |
| **ADDED** | C1743: Removing A2 restores Herbal thermal-intensity ordering -- H(A3):R4>R3 p=0.021 |
| **UPDATED** | INDEX.md -- +3 constraints (1742 total), Phase 601 section with substantive verdict |
| **UPDATED** | CLAUDE.md -- Quick reference updated (v5.74, 1742 constraints, 601 phases) |

### Key Findings

- **A2 is the mechanism, not forgivingness gradient (C1741):** A2 is a compound apparatus regime — high null close recovery (C1639), higher authenticity threshold (C1644), morphology-selective counterfeiting (C1645), weak events losing to null (C1642). Section-controlled A2 dummy coefficient=-0.124, t=-5.31, p=1.2×10⁻⁶, R²=0.579. A one-dimensional forgivingness coordinate cannot capture this compound structure. This is the mechanism behind the H:R2 reversal (C1739).
- **Continuous forgivingness fails — informative negative (P1, P2):** P1 section-controlled rho=-0.047, p=0.686. P2 F=0.457, p=0.506. The data says the moderation is categorical (A2 vs non-A2), not parametric.
- **Removing A2 rescues Herbal (C1743):** H(A3):R4=0.101 > H(A3):R3=0.032, p=0.021. Phase 600 Herbal reversal was specifically A2 contamination. Caution: local ordering recovery, not a general REGIME law (C1730, C494).
- **Closure authenticity is the local mechanism (C1742):** strong_close_fraction → safety_balance partial rho=0.304, p=0.008. If closures are genuinely strong, preventive stabilization works; if weak/counterfeitable, transformative rescue. Connects C1642-C1647 to C1732-C1733.
- **DYE orthogonal to safety (S1a/S1b):** Different control axes. Stabilizes C1740, C1633-C1634.
- **Synthesis:** Brunschwig alignment survives as profile-conditioned safety doctrine — preventive stabilization where closure can be trusted, transformative rescue where closure authenticity is difficult to maintain.
...
- **Cross-section closure-demand bridge fails structurally (C1739):** 0/4 pre-registered tests pass. Bridge assumes monotone containment→preventive safety mapping, but Voynich closure manifold is profile- and threshold-dependent. H:R2 reversal (lowest ey 0.0604, highest ii 0.1136) is not random — consistent with authenticity-sensitive containment regimes loading onto transformative safety (C1639-C1647, C1733). Sealedness ≠ preventive safety discipline. Annotation quality stable (H1 rho=0.744, H2 rho=0.842).
- **Stars safety substitution confirmed at individual axis level (C1740):** ey_rate replicates C1735 (p=0.0003 vs original p=0.0007). ii_rate provides FIRST direct confirmation that safety substitution model (C1732/C1733) aligns with Brunschwig operational framework. DYE_advantage reverses — DYE is intervention-productivity (C1633-C1634), orthogonal to preventive/transformative safety axis. R3 closure phenotype is intervention-productive rather than stable-preventive.
- **Brunschwig alignment is narrower and more section-conditioned than broad multi-cell prototypes assumed.** Replicated alignment: thermal-intensity / safety-substitution within Stars (C1735, C1740) and within-folio THERMAL gradient (C1736). Failed cross-section bridges: C1737 (method-bundle→apparatus), C1739 (closure-demand→closure-response), Phase 598d (apparatus profiles within sections). Any historical bridge collapsing the closure manifold to a single containment axis is dead on arrival.
