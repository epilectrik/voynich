...
This project was built using AI-assisted computational analysis over 648 research phases. The primary development environment was [Claude Code](https://claude.ai/claude-code) (Anthropic), which wrote the analysis scripts, maintained the constraint system, and performed statistical validation. GPT-5 (OpenAI) provided independent cross-validation at key decision points. All claims are grounded in statistical evidence from the transcript data — no result depends on AI intuition or pattern-matching alone.
...
The result: knowledge compounds across phases. No individual analysis session could discover 49 instruction classes, 17 forbidden transitions, 6 macro states, an 18-atom instruction encoding architecture, the Brunschwig alignment, and 51 recipe-folio correspondences — but 648 phases, each building on validated prior work, could.
...
Total: 1,962 validated constraints.
...
Running [Claude Code](https://claude.ai/claude-code) in this repository automatically creates a Voynich expert with the full constraint system loaded as permanent context. The `CLAUDE.md` instructions, context architecture, and embedded agent definitions give the AI access to all 1,962 constraints, 6 structural contracts, and 75 model fits.
...
```
voynich/
  context/            # Constraint system (1,962 validated constraints)
