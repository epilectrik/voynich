...
| Ch15P | Alternative gold dissolution (alembic + lunaria) | f84v | cs hard-filter (recto/verso of f84r) |
...
- **The e-channel regex underrepresents cooling vocabulary.** The PL-side `correction_rate` feature (dimension 3) captures error/correction keywords (error, defect, burn, combust, beware, etc.) but does NOT capture direct cooling vocabulary (cool, refrigerate, temper, gentle heat, settle, stabilize). The V-side `e_ratio` measures the `e` atom (cool) which encodes thermal gentleness broadly, not just error-correction cooling. Testing on 8 sample chapters shows the gap adds 0-2 hits per chapter, roughly doubling the e-channel score on balneum-heavy recipes like Ch14P (gold dissolution) which scored 0.000 with the old regex despite being a water-bath recipe. The gap is unlikely to change matching assignments (relative ordering preserved) but it weakens the e-channel correlation. This will be addressed when the matching is re-run against the Catalan source text.

- **The matching was performed against a 1566 Latin reprint, not the original text.** The Testamentum was composed in Catalan in 1332 and translated to Latin in 1443. Our 1566 printed edition is 230+ years removed from the original. The Catalan text is more operationally explicit (workshop-level detail that the Latin translator flattened). Preliminary comparison of available Catalan chapters (Practica Ch1-12, Mercuriorum Ch1-14, from Buosi-Moncunill 2023 thesis) shows sharper token-level correspondence than the Latin. We have ordered the Pereira-Spaggiari 1999 SISMEL critical edition containing the complete Catalan text from Oxford CCC MS 244. When it arrives, we plan to re-run the full matching pipeline against the Catalan, which should resolve token count discrepancies, tighten the e-channel gap, and provide a definitive test of verb-level correspondence.
