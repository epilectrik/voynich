...
|--------|-------|
| Version | 7.11 |
| Constraints | 2055 validated (17 retracted, 2 retested, 17 demoted) |
| Phases | 748 completed |
| Folios | 83 (Currier B) |
...
**Session methodology memories auto-embedded:** `generate_expert_context.py` now pulls feedback-type memory notes from `~/.claude/projects/C--git-voynich/memory/` and embeds them in the expert-advisor agent under a "Session Methodology Notes" section. Project-type and other-type memories are NOT included (those are individual findings the expert can re-derive). Feedback memories document recurring trap patterns and discipline rules — when added, they become part of the expert's permanent context. Both `expert-advisor.md` AND `crazy-expert.md` are now generated by this script (crazy-expert prepends `context/CRAZY_EXPERT_STANCE.md` to the same shared body), so both auto-inherit the methodology section — no manual sync needed.
...
The crazy-expert agent (`.claude/agents/crazy-expert.md`) is **now generated by `generate_expert_context.py`** (converted 2026-05-30). It prepends the hand-curated speculation stance in `context/CRAZY_EXPERT_STANCE.md` to the SAME shared body as expert-advisor, so it **auto-inherits** all constraints, fits, Tier 3-4 interpretations, and the (trimmed) methodology section. **No manual constraint or methodology sync is needed** — running the step-5 generator regenerates both agents.
- To change crazy-expert's speculation **persona/stance**, edit `context/CRAZY_EXPERT_STANCE.md` and regenerate.
- The old manual tab-separated constraint-append process is RETIRED.
