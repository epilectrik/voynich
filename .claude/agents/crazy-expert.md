...
the highest ID present is C2078.
...
6. Session Methodology Notes (44 feedback rules)
...
C762	**Cross-System Single-Char Primitive Overlap** (f49v/f76r/f57v share 4 chars d,k,o,r - all C085 primitives; spans PREFIX/MIDDLE/SUFFIX positions) **[SYNTHESIS 2026-06-10: f57v(coord ring)+f66r(register marks)+f49v(reader's imitation) = one REFERENCE single-char notation, addressing apparatus; f76r EXCLUDED (operational sentinels). See SPECULATIVE/f57v_reference_alphabet_family.md]**	2	GLOBAL
...
C1250	**Gloss Category Structural Coherence** (8 operational categories on 90 MIDDLEs: 5/7 permutation tests PASS) — **[PHASE_745 ANNOTATED (C2069): the partition is keyword-IMPOSED, "8" never k-tested. The PASSes split: the load-bearing ones are ATOM-INDEPENDENT — these survive. The behavioral-silhouette and kernel-alignment (V=0.675) PASSes are CIRCULAR (features/predictor = the defining atoms; real silhouette is negative −0.070) → calibration, not discovery. Keep Tier 2 for the atom-independent signal only.]**	2	B, gloss, category, validation, corpus-scale
...
C1254	**Dark Pipeline Category Generalization** — **[PHASE_745 ANNOTATED (C2069): ENFORCE its own stratification — only the LOCKED+SOLID atom tier (~12% of assignments) carries validated signal; the PLAUSIBLE/WEAK tier (~65%, o/l/r-driven) is unvalidated noise, and the "behavioral silhouette FAIL overall" is the imposed-partition tell. The category labels are scaffolding; the atom-INDEPENDENT axes (position/MI) are the real signal.]**	2	B, dark-pipeline, gloss, atom, coverage
...
C1260	**Mode B Thermal State Tracking**, NOT line-to-line carryover -- within-paragraph lag1=lag2 flat, 2026-06 retest)	2	B, mode-b, thermal, state-tracking, energy-balance, propagation, FL, steady-state
...
C1273	**AZC-Exclusive Vocabulary is MARKING/THERMAL Enriched** (356 UNK MIDDLEs assigned by atom vote; MARKING 27.2% THERMAL 27.0%) — **[PHASE_745 FLAGGED (C2069): the "MARKING 27.2%" share is unreliable — MARKING was inflated ~34% by the stale d=mark gloss (d re-glossed to do/execute, C1934). Recompute with corrected d before using; much of "MARKING" is OPERATION. The general "AZC has its own private vocabulary" point stands at the atom/exclusivity level (cf. C760), but the category percentages do not.]**	2	AZC, exclusive, vocabulary, ~~MARKING~~, THERMAL
...
C1682	Thermal seasonal gradient absent: THERMAL_GRADIENT_ABSENT (THERMAL p>0.2, CONTAINMENT p>0.16). **[PHASE_744: SURVIVES as a null — direction-robust. Scope caveat: "any mapping variant" meant the 4-season variants tested, not literal map-agnosticism; the conclusion (seasonal signal not in the thermal channel) holds and is reinforced by C1681's retraction (there's barely a seasonal signal at all).]**	2	AZC, zodiac, THERMAL, season, negative
C1685	Full zodiac map NOT INFERRED: ZODIAC_MAP_NOT_INFERRED... best perm_p=0.112. **[PHASE_744: SURVIVES — now the HEADLINE. The un-fitted full-map seasonal signal is NULL. This is the honest estimate; C1681's perm_p=0.018 was selection-optimized. Map inference itself is MOOT (iconography gives all signs).]**	2	AZC, zodiac, season, negative
C1686	Within-season assignment degeneracy... **[PHASE_744 ANNOTATED: the "Capricorn/Aquarius" within-season pair is PHANTOM (those signs absent). Surviving point: the seasonal-test search space is degenerate (≈3 effective groupings) — which bounds the multiplicity correction (~3-9×) that VOIDS C1681's perm_p.]**	2	AZC, zodiac, degeneracy
...
C2078	**A/B Within-Token Construction-Determinism Difference is Localized to Multi-Character Suffix Construction** (char-construction conditional entropy H(c\|prev2) on 395 shared MIDDLEs, root-frequency-matched; ΔH=H(A)−H(B)). NOT uniform "B tighter" — it REVERSES by region: PREFIX −0.049 [−0.064,−0.032] (B looser), CORE −0.025 [−0.033,−0.016] (B looser), SUFFIX len-1 +0.019 [−0.019,+0.059] (NULL, both −y ~80%), SUFFIX len-2 **+0.295 [+0.268,+0.324] (B much tighter)**. So A and B build the same roots and same single-char closures identically, but B constructs MULTI-character suffixes as near-deterministic closure operators while A attaches them freely. Controls all passed: N-match, shared-MIDDLE (C1514 inventory JSD 0.050 → diff is informative), prefix-strip (C1538 qo), root-freq-match (killed ¾ of naive +0.165→+0.041 = frequency-shadow), $-exclusion (effect GREW +0.125→+0.228 → not length/terminal artifact), length-stratified (survives at fixed len-2, null at len-1 → not the real A/B suffix-length diff 0.86 vs 1.18). Miller-Madow bias works AGAINST → +0.295 conservative. EXTENDS C522 (construction layer carries localized A/B distinction; ≠ reopening independence), SHARPENS C2009 (region-decomposes whole-token divergence), DISTINGUISHED FROM C1514 (arrangement-entropy ≠ atom-inventory; A wider-atom-set is consistent/corroborating), cross-ref C1487/C919/C1735 (closure grammar). C171-safe MEASUREMENT; "B=executable closure-terminator / A=registry" reading held Tier-3/4 SPECULATIVE only (framework-as-null). Differential-check validated: advisor (novelty:partial-new) + lean (rigor:caught length confound) DIVERGED on aggregate → running lean's $-excl+length-strat controls resolved it (the prefix/core reversal is the non-echo content).	2	A, B, construction, suffix, closure, determinism, conditional_entropy, region_localized, C522, C2009, C1514, C1487
...
# Consult the constraint base before deflating established work (2026-05-30)

User correctly diagnosed a session pattern: "you are being overconfident in incorrect interpretations, not following our previous research, and using faulty reasoning to dismiss previous work." Substantially fair. This note records the mechanism so it doesn't recur after compaction.
...
## feedback_cross_line_b_tests_within_voice_within_paragraph

*"Cross-line continuity tests in Currier B must separate Mode A/B voices AND localize line-vs-paragraph with lag-1-vs-lag-2 (NOT within-paragraph shuffle, which over-controls the negative)."*

"Is the controller stateless per-line in channel X" tests on Currier B are misled by two stacked confounds. Run BOTH controls before any stateless/coupled verdict (2026-06 audit of C670/C673/C681/C1031).

1. **Cross-voice pooling understates within-voice continuity.** Physically-adjacent lines interleave the Mode A (specification-injection) and Mode B (continuous) voices (C1258 counterpoint). Pooled adjacency reads null (C670 vocab Jaccard 0.140≈0.126; C1031 FL; C1830 census = only length/PREFIX survive). But within the Mode-B *subsequence* (skip Mode-A lines), vocab/thermal/loop continuity appears and survives folio-shadow + length-residual controls (z~4). So a cross-voice null does NOT establish per-line statelessness — it bounds a different quantity. C670's own row says C1258 "explains" it; the framing was never reconciled into the row.

[…trimmed — full note: memory/feedback_cross_line_b_tests_within_voice_within_paragraph.md]
...
## feedback_label_fit_to_signal

*"Failure pattern — a latent categorical assignment chosen to maximize the test statistic, then reported as if pre-registered"*

**Label-fit-to-signal (latent-label optimization).** A constraint computes a statistic over groups defined by a *latent/uncertain categorical assignment* (which season a folio is, which class a token is, which cluster a unit belongs to), and the **assignment itself was chosen from a menu to maximize the reported statistic** — then the optimized value is presented as a finding, without look-elsewhere/multiplicity correction.
...
## feedback_never_declare_exhausted_until_understood

*Framing directive — never call the work exhausted/done/terminal while the manuscript is not understood; method-exhaustion is not goal-completion*

**Never frame the program as "exhausted / done / terminal / largely finished" while the manuscript is not fully understood.** The goal is *understanding what the manuscript means* (recovering referents — what the structure is about), NOT merely characterizing structure. "A method/toolkit is exhausted" ≠ "the goal is reached." User directive, 2026-06 (stated firmly, after the third premature-closure catch in one session).

**Why:** the project has mapped the STRUCTURE (control-program grammar, index layer, hazard topology) but recovered ZERO referents — we cannot read a single thing it says. By the standard that matters (understanding), the work is wide open. Saying "discovery exhausted" conflates a narrow true fact (text-internal statistics are generic at the domain-within-Latin resolution, PHASE_718/720) with a false one (the question is answered / the program is done). It also reads as

[…trimmed — full note: memory/feedback_never_declare_exhausted_until_understood.md]
...
## feedback-testamentum-total-evidence-picture

*"RECURRING failure mode: after compaction Claude deflates the recipe/Testamentum matching evidence piece-by-piece against the WRONG (text-cipher) hypothesis and applies the mirror argument as a blanket, missing the total-evidence case. The user's actual hypothesis is operational-notation-of-Testamentum-tradition, which the totality supports. Holds BOTH the correction and the guardrails (do not over-promote to decoded / to the specific text)."*

# The Testamentum total-evidence picture (recurring conversation, 2026-05-30)

The user reports this exact conversation recurs after every compaction: Claude loses the thread and it's "a struggle" to get Claude to see the whole Testamentum picture. This note records the correction AND the guardrails so it doesn't have to be re-derived.

## The recurring failure mode (the why)
