...
Runs 10 baselines (B1-B10) + 4 null models (N1-N4, 50 perms each) for the
20 pilot folios through the close recovery apparatus.

Total: 10 baselines x 20 folios + 4 nulls x 20 folios x 50 perms
     = 200 + 4000 = 4,200 runs + 20 reference = 4,220 runs

B1-B8: Modified apparatus or inputs (all use CloseRecoveryApparatus)
B9: UniformRestoringApparatus (flat gamma, but CLOSE recovery intact)
B10: NoCloseRecoveryApparatus (CLOSE recovery disabled, K_CTS=2.0)

N1-N4: Permutation null models (50 permutations each)
...
T1_566_PATH = PHASE_DIR / 'results' / 't1_close_recovery_apparatus.json'
...
# ---------------------------------------------------------------------------
# UniformRestoringApparatus for B9 (wraps CloseRecoveryApparatus)
# ---------------------------------------------------------------------------
class UniformRestoringApparatus:
    ...
    CLOSE recovery channels (R1-R5) remain intact -- B9 tests zone
    geometry, not recovery architecture.
    """
    ...
    def update(self, state, dV, packet_phase='WORK', cts=0.0, permissivity=None):
        """
        Same as CloseRecoveryApparatus.update but with uniform restoring.
        Steps:
          1. Cross-coupling (from base)
          2. Uniform restoring (replaces piecewise zones)
          3. Discharge events (from base)
          4. State update
          5. CLOSE recovery R1-R5 (from base, intact)
        """
        ...
        # 3. Discharge events (same as base -- uses overridden version
        #    that suppresses old CTS/containment when CLOSE recovery active)
        ...
        # 5. CLOSE recovery R1-R5 (pass dV magnitude for clean-CLOSE Y bonus)
...
# ---------------------------------------------------------------------------
# Core execution function (adapted for CloseRecoveryApparatus)
# ---------------------------------------------------------------------------
def run_close_recovery_trace(apparatus, tokens, line_packets,
    ...
    Parameters
    ----------
    apparatus : CloseRecoveryApparatus, UniformRestoringApparatus, or similar
    ...
    work_deviation_change, close_deviation_change
    """
    n_tokens = len(tokens)
    if n_tokens == 0:
        return {
            ...
            'work_deviation_change': [0.0] * N_VARS,
            'close_deviation_change': [0.0] * N_VARS,
        }
    ...
    # Per-SV warning and hard_stop contact counts
    warning_contacts_per_sv = {sv: 0 for sv in STATE_VARS}
    hard_stop_contacts_per_sv = {sv: 0 for sv in STATE_VARS}
    ...
    # Deviation change tracking per phase
    work_dev_sum = [0.0] * N_VARS
    work_count = 0
    close_dev_sum = [0.0] * N_VARS
    close_count = 0

    # Corridor return latency tracking
    corridor_return_events = []
    # Track per-SV excursion state: {sv: {'active': bool, 'start_idx': int, 'peak_dev': float}}
    sv_excursion_state = {sv: {'active': False, 'start_idx': 0, 'peak_dev': 0.0}
                          for sv in STATE_VARS}
    ...
    for tok_idx, tok in enumerate(tokens):
        ...
        # Save pre-update state for deviation change tracking
        ...
        if disable_discharge:
            ...
            if hasattr(apparatus, 'base'):
                ...
                # No discharge, but still apply CLOSE recovery
                new_state = []
                for i in range(N_VARS):
                    v = state[i] + dV[i] + cc[i] - rf[i]
                    new_state.append(max(0.0, min(1.0, v)))
        ...
        for sv in STATE_VARS:
            ...
            if z == 'WARNING':
                ...
                warning_contacts_per_sv[sv] += 1
            elif z == 'HARD_STOP':
                token_has_hard_stop = True
                hard_stop_contacts_per_sv[sv] += 1
        ...
        # 11. Deviation change tracking per phase
        for i in range(N_VARS):
            dev_change = abs(state[i] - EQUILIBRIUM) - abs(pre_state[i] - EQUILIBRIUM)
            if packet_phase == 'WORK':
                work_dev_sum[i] += dev_change
            elif packet_phase == 'CLOSE':
                close_dev_sum[i] += dev_change
        ...
        # 12. Corridor return latency tracking (per-SV)
        for sv in STATE_VARS:
            i = SV_INDEX[sv]
            dev_abs = abs(state[i] - EQUILIBRIUM)
            q2_sv = Q2_BASE[sv]

            if not sv_excursion_state[sv]['active']:
                # Enter excursion when deviation exceeds Q2
                if dev_abs > q2_sv:
                    sv_excursion_state[sv]['active'] = True
                    sv_excursion_state[sv]['start_idx'] = tok_idx
                    sv_excursion_state[sv]['peak_dev'] = dev_abs
            else:
                # Track peak
                if dev_abs > sv_excursion_state[sv]['peak_dev']:
                    sv_excursion_state[sv]['peak_dev'] = dev_abs
                # Return when back within Q1 (basin)
                if dev_abs < Q1:
                    latency = tok_idx - sv_excursion_state[sv]['start_idx']
                    corridor_return_events.append({
                        'sv': sv,
                        'excursion_token_idx': sv_excursion_state[sv]['start_idx'],
                        'return_token_idx': tok_idx,
                        'latency': latency,
                        'peak_dev': round(sv_excursion_state[sv]['peak_dev'], 5),
                    })
                    sv_excursion_state[sv]['active'] = False

        # 13. Excursion cycle tracking
    ...
    # Compute mean deviation changes
    work_deviation_change = [round(work_dev_sum[i] / work_count, 6)
                             if work_count > 0 else 0.0
                             for i in range(N_VARS)]
    close_deviation_change = [round(close_dev_sum[i] / close_count, 6)
                              if close_count > 0 else 0.0
                              for i in range(N_VARS)]
    ...
    return {
        ...
        'warning_contacts_per_sv': warning_contacts_per_sv,
        'hard_stop_contacts_per_sv': hard_stop_contacts_per_sv,
        'work_deviation_change': work_deviation_change,
        'close_deviation_change': close_deviation_change,
        'corridor_return_latency': corridor_return_events,
    }
...
def null_n1_phase_shuffle(tokens, rng):
    ...
    Shuffles ALL tokens within a folio (across lines), destroying
    phase-token alignment while preserving magnitude distribution."""
...
def null_n2_contribution_shuffle(tokens, rng):
    ...
    Preserves total magnitude but scrambles which SVs receive which
    contributions."""
...
def null_n3_cross_folio(tokens, all_tokens_by_folio, rng, target_folio):
    """N3: Cross-folio. Apply folio A's supervisory signal to folio B's
    apparatus. Returns tokens from a randomly selected different folio,
    adjusted to match the target folio's positional structure."""
    available_folios = [f for f in all_tokens_by_folio
                        if f != target_folio and len(all_tokens_by_folio[f]) > 0]
    if not available_folios:
        return tokens  # Fallback: return original

    source_folio = rng.choice(available_folios)
    source_toks = all_tokens_by_folio[source_folio]

    # Use source folio's contributions but target folio's positional structure
    n_target = len(tokens)
    ...
    for i, orig_tok in enumerate(tokens):
        nt = dict(orig_tok)
        # Pull contributions from source folio (wrapping if needed)
        src_idx = i % n_source
...
def null_n4_random_walk(tokens, rng):
    """N4: Random walk. Random dV magnitudes with same distribution as
    original. Preserves the statistical distribution of contribution
    magnitudes but randomizes their assignment."""
    # Collect all contribution values across all SVs
    all_values = []
    for tok in tokens:
    ...
    shuffled = [dict(t) for t in tokens]
    # Shuffle the entire pool of values
    rng.shuffle(all_values)

    # Redistribute into tokens
    idx = 0
    for tok in shuffled:
        new_contribs = []
...
def main():
    ...
    for folio in pilot_folio_list:
        ...
        # Config mode from 566 T1 infra scores
    ...
    for folio in pilot_folio_list:
        ...
        result = run_close_recovery_trace(apparatus, toks, line_packets)
    ...
    for folio in pilot_folio_list:
        ...
        # --- B1: Uniform sensitivity (folio-mean contributions) ---
        b1_toks = []
        for t in toks:
            nt = dict(t)
            ...
            b1_toks.append(nt)
        apparatus = build_configured_apparatus(profile, config_mode)
        r = run_close_recovery_trace(apparatus, b1_toks, line_packets)
        r['folio'] = folio
        r['profile'] = profile
        ...
        for t in toks:
            ...
            b2_toks.append(nt)
        apparatus = build_configured_apparatus(profile, config_mode)
        r = run_close_recovery_trace(apparatus, b2_toks, line_packets)
        r['folio'] = folio
        r['profile'] = profile
        ...
        # --- B3: Shuffled phase labels (no CTS) ---
        apparatus = build_configured_apparatus(profile, config_mode)
        r = run_close_recovery_trace(apparatus, toks, line_packets, disable_cts=True)
        r['folio'] = folio
        r['profile'] = profile
        ...
        # --- B4: Random profile assignment (rotated) ---
        ...
        r = run_close_recovery_trace(apparatus, toks, line_packets)
        r['folio'] = folio
        ...
        # Build apparatus then zero out cross-coupling by zeroing profile params
        apparatus = build_configured_apparatus(profile, config_mode)
        # Override profile_params to zero all alpha terms
        for key in list(apparatus.profile_params.keys()):
            if key.startswith('alpha_'):
                apparatus.profile_params[key] = 0.0
        # Recompute equilibrium bias with zeroed cross-coupling
        ...
        # --- B9: Uniform restoring (no piecewise zones, CLOSE recovery intact) ---
        base_apparatus = build_configured_apparatus(profile, config_mode)
        uniform_apparatus = UniformRestoringApparatus(base_apparatus)
        r = run_close_recovery_trace(uniform_apparatus, toks, line_packets)
        r['folio'] = folio
        r['profile'] = profile
        ...
        # --- B10: No CLOSE recovery (reverts to 565 discharge levels) ---
        ...
        run_count += 1
        print(f"  B10 {folio}... done (viab={r['viability']:.4f})")
    ...
    for folio in pilot_folio_list:
        ...
        for null_name in null_runs:
            null_runs[null_name][folio] = {
                ...
                'warning_contacts': [],
                'hard_stop_contacts': [],
                'work_deviation_changes': [],
                'close_deviation_changes': [],
            }
        ...
        for perm_idx in range(N_PERMS):
            ...
            apparatus = build_configured_apparatus(profile, config_mode)
            r1 = run_close_recovery_trace(apparatus, n1_toks, line_packets)
            ...
            null_runs['N1'][folio]['work_deviation_changes'].append(r1['work_deviation_change'])
            null_runs['N1'][folio]['close_deviation_changes'].append(r1['close_deviation_change'])
            run_count += 1
            ...
            apparatus = build_configured_apparatus(profile, config_mode)
            r2 = run_close_recovery_trace(apparatus, n2_toks, line_packets)
            ...
            null_runs['N2'][folio]['work_deviation_changes'].append(r2['work_deviation_change'])
            null_runs['N2'][folio]['close_deviation_changes'].append(r2['close_deviation_change'])
            run_count += 1
            ...
            apparatus = build_configured_apparatus(profile, config_mode)
            r3 = run_close_recovery_trace(apparatus, n3_toks, line_packets)
            ...
            null_runs['N3'][folio]['work_deviation_changes'].append(r3['work_deviation_change'])
            null_runs['N3'][folio]['close_deviation_changes'].append(r3['close_deviation_change'])
            run_count += 1
            ...
            apparatus = build_configured_apparatus(profile, config_mode)
            r4 = run_close_recovery_trace(apparatus, n4_toks, line_packets)
            ...
            null_runs['N4'][folio]['work_deviation_changes'].append(r4['work_deviation_change'])
            null_runs['N4'][folio]['close_deviation_changes'].append(r4['close_deviation_change'])
            run_count += 1
    ...
    for null_name in null_runs:
        for folio in null_runs[null_name]:
            ...
            # Mean deviation changes across permutations
            mean_work_dev = [0.0] * N_VARS
            mean_close_dev = [0.0] * N_VARS
            for perm_work in entry['work_deviation_changes']:
                for i in range(N_VARS):
                    mean_work_dev[i] += perm_work[i]
            for perm_close in entry['close_deviation_changes']:
                for i in range(N_VARS):
                    mean_close_dev[i] += perm_close[i]
            entry['mean_work_deviation_change'] = [round(v / n_p, 6) for v in mean_work_dev]
            entry['mean_close_deviation_change'] = [round(v / n_p, 6) for v in mean_close_dev]

            # Round raw arrays for storage
            entry['viabilities'] = [round(v, 6) for v in viabs]
            entry['y_finals'] = [round(v, 6) for v in y_finals]
            # Remove the per-perm deviation change lists to save space
            del entry['work_deviation_changes']
            del entry['close_deviation_changes']
    ...
    # Strip corridor_return_latency from reference for JSON size
    # (keep summary statistics but drop per-event detail)
    reference_output = {}
    for folio, ref_data in reference.items():
        ref_copy = dict(ref_data)
        crl = ref_copy.pop('corridor_return_latency', [])
        ref_copy['corridor_return_count'] = len(crl)
        if crl:
            latencies = [e['latency'] for e in crl]
            ref_copy['corridor_return_mean_latency'] = round(sum(latencies) / len(latencies), 2)
            ref_copy['corridor_return_max_latency'] = max(latencies)
        else:
            ref_copy['corridor_return_mean_latency'] = 0.0
            ref_copy['corridor_return_max_latency'] = 0
        reference_output[folio] = ref_copy

    # Strip corridor_return_latency from baselines too
    for bname in baseline_runs:
        for entry in baseline_runs[bname]:
            crl = entry.pop('corridor_return_latency', [])
            entry['corridor_return_count'] = len(crl)
            if crl:
                latencies = [e['latency'] for e in crl]
                entry['corridor_return_mean_latency'] = round(
                    sum(latencies) / len(latencies), 2)
                entry['corridor_return_max_latency'] = max(latencies)
            else:
                entry['corridor_return_mean_latency'] = 0.0
                entry['corridor_return_max_latency'] = 0
    ...
    print(f"  {'Folio':<8} {'Ref':>7} {'B10':>7} {'Delta':>7}")
    print(f"  {'-' * 8} {'-' * 7} {'-' * 7} {'-' * 7}")
    b10_deltas = []
    for folio in pilot_folio_list:
        if folio not in reference:
            continue
        ref_v = reference[folio]['viability']
        b10_v = baseline_by_folio['B10'].get(folio, 0.0)
        delta = ref_v - b10_v
        b10_deltas.append(delta)
        print(f"  {folio:<8} {ref_v:>7.4f} {b10_v:>7.4f} {delta:>+7.4f}")
    if b10_deltas:
        mean_delta = sum(b10_deltas) / len(b10_deltas)
        min_delta = min(b10_deltas)
        max_delta = max(b10_deltas)
        print(f"  {'MEAN':<8} {'':>7} {'':>7} {mean_delta:>+7.4f}")
        print(f"  {'MIN':<8} {'':>7} {'':>7} {min_delta:>+7.4f}")
        print(f"  {'MAX':<8} {'':>7} {'':>7} {max_delta:>+7.4f}")

    # --- B9 Delta Analysis (uniform restoring) ---
    print(f"\n  B9 Deltas (Reference - B9, uniform restoring):")
    ...
    # --- WORK vs CLOSE deviation change comparison ---
    print(f"\n  WORK vs CLOSE Deviation Change (Reference):")
    print(f"  {'Folio':<8} {'W_mean':>8} {'C_mean':>8} {'Asymm':>8}")
    print(f"  {'-' * 8} {'-' * 8} {'-' * 8} {'-' * 8}")
    for folio in pilot_folio_list:
        if folio not in reference:
            continue
        w_dev = reference[folio]['work_deviation_change']
        c_dev = reference[folio]['close_deviation_change']
        w_mean = sum(w_dev) / len(w_dev)
        c_mean = sum(c_dev) / len(c_dev)
        asymm = w_mean - c_mean
        print(f"  {folio:<8} {w_mean:>8.5f} {c_mean:>8.5f} {asymm:>+8.5f}")
