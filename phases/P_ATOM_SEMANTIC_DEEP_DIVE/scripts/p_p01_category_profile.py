#!/usr/bin/env python3
"""
P-P1: p-initial 8-category profile

Test whether p-initial MIDDLEs are enriched in MARKING category.
p = "pause" maps to MARKING via CategoryClassifier. The MEDIAL slot
position (C1209) and positive carryover (C1208, z=+4.27) suggest p acts
as an operational pause/marking element.

Predictions:
  1. p-initial MARKING enrichment >= 1.5x
  2. p-initial THERMAL depletion <= 0.6x
  3. p ranked top 5 for MARKING among all initial atoms
  4. Per-section stability: MARKING enriched >= 1.2x in >= 3/5 sections

Pass criterion: MARKING >= 1.5x AND THERMAL <= 0.6x (primary),
               plus >= 3/4 sub-criteria total for full pass.

Controls: k-initial (THERMAL), c-initial (MONITORING), d-initial (MARKING)
"""
...
def main():
    ...
    print("=" * 70)
    print("P-P1: p-initial MARKING category enrichment")
    print("=" * 70)
    print()
    ...
    for rank, (atom, frac, mark_n, n, enrich) in enumerate(atom_marking, 1):
        marker = " <-- p-initial" if atom == 'p' else ""
    ...
    if p_rank is not None:
        print("  p-initial rank: #%d of %d atoms" % (p_rank, len(atom_marking)))
    else:
        print("  WARNING: p-initial not found or too few tokens")

    # ---- TEST 2: Chi-square test for MARKING ----
    print()
    print("-" * 70)
    print("TEST 2: Chi-square -- p-initial MARKING vs global rate")
    print("-" * 70)

    p_val_marking = 1.0
    chi2_marking = 0.0
    if p_rank is not None and p_total > 0:
        a = p_mark_n
        b = p_total - p_mark_n
        c_other = global_cat_counts['MARKING'] - p_mark_n
        d = (global_total - p_total) - c_other
        total = a + b + c_other + d
    ...
    # ---- TEST 3: Full 8-category profile for p-initial ----
    print()
    print("-" * 70)
    print("TEST 3: Full 8-category profile for p-initial MIDDLEs")
    print("-" * 70)

    p_thermal_enrichment = 0.0
    if p_rank is not None and p_total > 0:
        print("%-15s %11s %9s %11s %7s" % ('Category', 'p-initial%', 'Global%', 'Enrichment', 'Count'))
        print("-" * 55)
        for cat in CATEGORIES:
            p_cat_n = atom_cat_counts['p'].get(cat, 0)
            p_cat_frac = p_cat_n / p_total if p_total > 0 else 0
            global_frac = global_cat_counts[cat] / global_total if global_total > 0 else 0
            enrich = p_cat_frac / global_frac if global_frac > 0 else 0
            marker = ""
            if cat == 'MARKING':
                marker = "  <-- predicted enriched"
            elif cat == 'THERMAL':
                marker = "  <-- predicted depleted"
                p_thermal_enrichment = enrich
            print("  %-13s %10.2f%% %8.2f%% %10.3fx %7d%s" % (cat, p_cat_frac * 100, global_frac * 100, enrich, p_cat_n, marker))

    # ---- TEST 4: Per-section stability ----
    print()
    print("-" * 70)
    print("TEST 4: p-initial MARKING enrichment per section")
    print("-" * 70)

    sections_tested = 0
    sections_enriched = 0

    for section in sorted(atom_section_totals.get('p', {}).keys()):
        p_sec_total = atom_section_totals['p'][section]
        p_sec_mark = atom_section_cat['p'][section].get('MARKING', 0)
        if p_sec_total < 10:
            continue
        ...
        p_rate = p_sec_mark / p_sec_total
        enrich = p_rate / sec_rate if sec_rate > 0 else 0
        ...
        print("  Section %8s: p-initial %d/%d = %.1f%%, "
              "section baseline %.1f%%, enrichment %.3fx [%s]" %
              (section, p_sec_mark, p_sec_total, p_rate * 100, sec_rate * 100, enrich, status))

    print()
    print("  Sections with MARKING enrichment >= 1.2x: %d/%d" % (sections_enriched, sections_tested))
    pass_section = sections_enriched >= 3

    # ---- CONTROLS: Compare to k-initial (THERMAL), c-initial (MONITORING), d-initial (MARKING) ----
    print()
    print("-" * 70)
    ...
    print()
    print("=" * 70)
    print("SUMMARY: P-P1 p-initial MARKING category enrichment")
    print("=" * 70)

    pass_marking = p_enrichment >= 1.5 if p_rank is not None else False
    pass_thermal = p_thermal_enrichment <= 0.6 if p_rank is not None else False
    pass_rank = p_rank is not None and p_rank <= 5
    pass_chi2 = p_val_marking < 0.01 if p_rank is not None else False

    results = [
        ("MARKING enrichment >= 1.5x", pass_marking,
         "%.3fx" % p_enrichment if p_rank else "N/A"),
        ("THERMAL depletion <= 0.6x", pass_thermal,
         "%.3fx" % p_thermal_enrichment if p_rank else "N/A"),
        ("Ranked top 5 for MARKING", pass_rank,
         "rank #%d" % p_rank if p_rank else "N/A"),
    ...
    print()
    print("  PRIMARY CRITERION (MARKING >= 1.5x AND THERMAL <= 0.6x): %s" % ('PASS' if primary else 'FAIL'))
