...
20 pilot folios through the selective restoration apparatus.
...
Input:
  ...
  - t1_selective_restoration_apparatus.json  (apparatus spec, folio infra scores)
  - t1_event_gated_apparatus.json            (folio profile assignments)
...
# ---------------------------------------------------------------------------
# Import selective restoration apparatus (Phase 564b T1)
# ---------------------------------------------------------------------------
...
from t1_selective_restoration_apparatus import (
    ...
    PILOT_FOLIOS, GAMMA_CORRIDOR, CORRIDOR_MULT, Q1, Q2_BASE,
    GAMMA_BASIN, BETA, BASIN_MULT, EDGE_MULT,
    PROFILE_DECAYS, A3_DECAY, CONFIG_MODES, K_RELIEF,
...
from t1_apparatus_family_builder import PROFILES
...
T1_564B_PATH = PHASE_DIR / 'results' / 't1_selective_restoration_apparatus.json'
T1_564_PATH = (PROJECT_ROOT / 'phases' / 'VIRTUAL_APPARATUS_EVENT_DYNAMICS'
               / 'results' / 't1_event_gated_apparatus.json')
...
# ---------------------------------------------------------------------------
# UniformRestoringApparatus for B9
# ---------------------------------------------------------------------------
class UniformRestoringApparatus:
    """
    Wraps SelectiveRestorationApparatus but replaces the 3-zone piecewise
    restoring force with uniform corridor-level restoring everywhere.

    No basin weakness, no edge barrier.
    rf = gamma_corridor[i] * dev * corridor_mult[phase][i]
    ...
    def update(self, state, dV, packet_phase='WORK', cts=0.0, permissivity=None):
        """
        Same as SelectiveRestorationApparatus.update but with uniform restoring.
        """
...
# ---------------------------------------------------------------------------
# Core execution function (adapted from Phase 564 T2 for SelectiveRestorationApparatus)
# ---------------------------------------------------------------------------
def run_selective_trace(apparatus, tokens, line_packets,
    ...
    """
    Run one folio through the selective restoration apparatus with routing buffer.

    Parameters
    ----------
    apparatus : SelectiveRestorationApparatus or UniformRestoringApparatus
    ...
    Returns
    -------
    dict with viability, Y_final, hazard counts, excursion tracking, zone occupancy
    """
    n_tokens = len(tokens)
    if n_tokens == 0:
        return {
    ...
    # Zone occupancy tracking: per SV count how many tokens in each zone
    zone_counts = {sv: {'BASIN': 0, 'CORRIDOR': 0, 'EDGE': 0}
    ...
    for tok_idx, tok in enumerate(tokens):
        ...
        if disable_discharge:
            ...
            # We use the base apparatus but override the discharge step
            # For simplicity: call update normally, then check if we need
            # to undo discharge. But the cleanest is to just set cts=0
            # and handle discharge separately. Since discharge only fires
            # in CLOSE phase, we can skip it by forcing cts=0 and wrapping.
            # Actually, discharge involves CTS discharge + containment +
            # thermal recovery. Setting cts=0 only removes CTS discharge.
            # The cleanest approach: temporarily patch discharge.
            # Instead, we call the internal methods directly.
    ...
    # Build zone occupancy summary (fractions)
    zone_occupancy = {}
    for sv in STATE_VARS:
...
def main():
    ...
    with open(T1_564B_PATH, 'r', encoding='utf-8') as f:
        t1b_data = json.load(f)
    infra_scores = t1b_data['folio_infra_scores']
    print(f"  564b infra scores: {len(infra_scores)} folios")

    with open(T1_564_PATH, 'r', encoding='utf-8') as f:
        t1_data = json.load(f)
    folio_assignments = t1_data['folio_assignments']
    print(f"  564 folio assignments: {len(folio_assignments)}")
    ...
    for folio in pilot_folio_list:
        # Preferred profile from Phase 564 T1 folio assignments
        ...
        # Config mode from 564b T1 infra scores
    ...
    for folio in pilot_folio_list:
        ...
        result = run_selective_trace(apparatus, toks, line_packets)
    ...
    for folio in pilot_folio_list:
        ...
        for t in toks:
            ...
            b1_toks.append(nt)
        apparatus = build_configured_apparatus(profile, config_mode)
        r = run_selective_trace(apparatus, b1_toks, line_packets)
        r['folio'] = folio
        r['profile'] = profile
        ...
        for t in toks:
            ...
            b2_toks.append(nt)
        apparatus = build_configured_apparatus(profile, config_mode)
        r = run_selective_trace(apparatus, b2_toks, line_packets)
        r['folio'] = folio
        r['profile'] = profile
        ...
        # --- B3: No CTS ---
        apparatus = build_configured_apparatus(profile, config_mode)
        r = run_selective_trace(apparatus, toks, line_packets, disable_cts=True)
        r['folio'] = folio
        r['profile'] = profile
        ...
        # --- B4: No routing ---
        apparatus = build_configured_apparatus(profile, config_mode)
        r = run_selective_trace(apparatus, toks, line_packets, disable_routing=True)
        r['folio'] = folio
        r['profile'] = profile
        ...
        r = run_selective_trace(apparatus, toks, line_packets)
        r['folio'] = folio
        r['profile'] = profile
        ...
        r = run_selective_trace(uniform_apparatus, toks, line_packets)
        r['folio'] = folio
        r['profile'] = profile
    ...
    for folio in pilot_folio_list:
        ...
        for perm_idx in range(N_PERMS):
            ...
            apparatus = build_configured_apparatus(profile, config_mode)
            r1 = run_selective_trace(apparatus, n1_toks, line_packets)
            ...
            apparatus = build_configured_apparatus(profile, config_mode)
            r2 = run_selective_trace(apparatus, n2_toks, line_packets)
            ...
            apparatus = build_configured_apparatus(profile, config_mode)
            r3 = run_selective_trace(apparatus, n3_toks, line_packets)
            ...
            apparatus = build_configured_apparatus(profile, config_mode)
            r4 = run_selective_trace(apparatus, n4_toks, line_packets)
