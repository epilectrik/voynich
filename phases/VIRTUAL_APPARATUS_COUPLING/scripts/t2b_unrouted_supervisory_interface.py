"""
T2b: Unrouted Supervisory Interface
=====================================
Phase 563b — ROUTING_ACCUMULATION_AND_DYNAMIC_RECALIBRATION

Recomputes the T2 supervisory interface WITHOUT routing effects baked into
contributions, and with halved headless magnitudes. Routing fields
(routing_active, routing_terminal) are preserved for T3b to consume.

Changes from T2:
  1. Routing effects NOT applied to raw_intent permissions.
     T3b applies routing as a cumulative within-line accumulator.
  2. Headless base magnitudes halved (base_c=0.15, base_s=0.10 at hl_norm=1.0).
  3. Everything else identical to T2.
...
OUTPUT = Path(__file__).resolve().parent.parent / "results" / "t2b_supervisory_interface_unrouted.json"
...
# Routing Effects (C1563) — kept for reference; T3b consumes these
ROUTING_EFFECTS = {
...
# -- Headless Intent Function (HALVED magnitudes) ----------------------------
...
def headless_intent(subtype, packet_phase, folio_hl_rate):
    ...
    T2b CHANGE: base magnitudes halved (0.15/0.10 vs T2's 0.30/0.20).
    """
    intent = {}
    # Scale with headless-heaviness (normalized to 0.25 as reference)
    hl_norm = folio_hl_rate / 0.25 if folio_hl_rate > 0 else 0.0
    base_c = 0.15 * hl_norm   # was 0.3 in T2
    base_s = 0.10 * hl_norm   # was 0.2 in T2
...
def main():
    t_start = time.time()
    print("=" * 70)
    print("T2b: Unrouted Supervisory Interface")
    print("=" * 70)
    ...
    for idx, tok in enumerate(tokens):
        ...
        # -- Detect routing (for T3b to consume) ------------------------------
        # NOTE: routing effects NOT applied to permissions here.
        # T3b applies routing as a cumulative within-line accumulator.
    ...
    output = {
        'metadata': {
            'phase': '563b',
            'task': 'T2b_unrouted_supervisory_interface',
            ...
            'changes_from_t2': [
                'routing_not_baked_into_contributions',
                'headless_magnitudes_halved',
            ],
        },
