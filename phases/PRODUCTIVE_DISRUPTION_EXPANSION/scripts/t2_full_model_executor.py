"""
T2: Full Model Executor — 2 Models x ~76 Folios
Phase 572 - PRODUCTIVE_DISRUPTION_EXPANSION

Runs M0 + M1 across all eligible folios from T1 setup (~152 total runs).
...
def load_data():
    ...
    # Load our T1 setup (Phase 572 full-scale)
    print("  Loading T1 setup (Phase 572 full-scale)...")
    setup_path = os.path.join(phases_dir, 'PRODUCTIVE_DISRUPTION_EXPANSION',
    ...
    print(f"    Eligible folios: {len(eligible_folios)} (of {len(all_folios)} total)")
    ...
    return (eligible_folios, all_folios, folio_configs,
...
def main():
    t0 = time.time()
    print("=" * 70)
    print("T2: Full Model Executor - 2 Models x ~76 Folios")
    print("Phase 572 - PRODUCTIVE_DISRUPTION_EXPANSION")
    print("=" * 70)

    # ---- Load data ----
    print("\n--- Loading data sources ---")
    (eligible_folios, all_folios, folio_configs,
     line_packets, cts_data, all_tokens, budgets, budget_path,
    ...
    # Build profile/config map for eligible folios
    folio_config = {}
    ...
    print("\n--- Extracting eligible folio tokens ---")
    ...
    # ================================================================
    # PRIMARY RUNS: ~76 folios x 2 models = ~152 total
    # ================================================================
    ...
    for folio in eligible_folios:
        ...
        print(f"  [{run_count:3d}/{total_runs}] {folio} M0:  "
        ...
        print(f"  [{run_count:3d}/{total_runs}] {folio} M1:  "
              f"PCV={m['PCV']:.4f} WCU={m['WCU']:.4f} "
              f"UEB={m['UEB']:6.1f} events={n_ev}")
