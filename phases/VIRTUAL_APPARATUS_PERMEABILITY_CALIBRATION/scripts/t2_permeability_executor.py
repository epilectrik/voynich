"""
T2: Permeability Calibration Executor
Phase 565 - VIRTUAL_APPARATUS_PERMEABILITY_CALIBRATION

Runs 20 pilot folios through the permeability apparatus (from T1)
...
Single-channel routing (same as 564b):
...
4-zone architecture (change from 564b):
  ...
  Zone 3b (HARD_STOP): |dev| >= q3          -- corridor + beta2 quadratic
"""
...
# ---------------------------------------------------------------------------
# Import T1 permeability apparatus
# ---------------------------------------------------------------------------
sys.path.insert(0, str(Path(__file__).resolve().parent))
from t1_permeability_apparatus import (
    PermeabilityApparatus, build_configured_apparatus,
    compute_infra_scores, STATE_VARS, Q1, Q2_BASE, Q3_BASE, HAZARD_BOUNDARIES,
    N_VARS, EQUILIBRIUM, SV_INDEX, PILOT_FOLIOS, HAZARD_DEV,
...
# Import PROFILES and assign_folio_profiles from apparatus family builder
...
def run_coupled_trace(apparatus, tokens, line_packets):
    """
    Run one folio through the permeability apparatus.

    apparatus:    PermeabilityApparatus instance
    ...
    for tok_idx, tok in enumerate(tokens):
        ...
        for sv in STATE_VARS:
            ...
            # Track warning and hard_stop contacts
            if zone == 'WARNING':
...
def main():
    ...
    for folio in PILOT_FOLIOS:
        ...
        for profile_name in PROFILE_NAMES:
            ...
            print(f"  [{run_count:2d}/60] {folio} + {profile_name} [{config_mode}]: "
                  ...
                  f"cycles={exc}, bounded={bnd}, warn={warn_total}, hs={hs_total}{pref_tag}")
    ...
    if pref_viabilities:
        ...
        total_warn = sum(pref_warning_totals)
        total_hs = sum(pref_hardstop_totals)
        ...
        print(f"  Total warning contacts (preferred): {total_warn}")
        print(f"  Total hard_stop contacts (preferred): {total_hs}")

    # Zone occupancy summary (preferred runs only, 4-zone)
