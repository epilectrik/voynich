...
| Metric | Value |
|--------|-------|
| Validated constraints | 1,097 |
| Research phases completed | 447 |
...
```
voynich/
  context/            # Constraint system (1,097 validated constraints)
  ...
  phases/             # 447 completed research phases
...
3. **Context is always loaded.** Every new analysis session begins with the full constraint system available. The AI doesn't start from scratch — it starts from everything that has already been proven, disproven, or established. This means phase 447 benefits from all 1,097 constraints accumulated across the previous 446 phases.
...
This architecture is what allowed the project to reach conclusions that would be impossible in a single analytical pass. No individual analysis session could discover 49 instruction classes, 17 forbidden transitions, 6 macro states, and the Brunschwig alignment — but 447 phases, each building on validated prior work, could.
