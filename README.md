...
- 46 procedural chapters from the *Testamentum* (28 Mercuriorum + 18 Practica) match to ~29 folios, covering 87% of the expanded procedural content (53 chapters after reclassifying 6 higher Mercuriorum). 3 confirmed by independent structural evidence (f75r/Ch19, f76r/Ch18P, f84r/Ch14P); 5 via reverse-blind prediction; 3 via fch (mercury marker) hard-filter matching; 1 via cs (gold marker) hard-filter; the remainder at supported or speculative confidence
...
| Pseudo-Lullian source tradition | 46 procedural chapters → ~29 folios (87% of 53 procedural); 3 confirmed + 5 reverse-blind + 3 fch hard-filter + ~34 supported/speculative |
...
| 2 | **[RECIPE_MATCHING.md](RECIPE_MATCHING.md)** | How we matched 45 recipe chapters to ~29 folios — methods, validation, and full results |
...
| Recipe chapter coverage | 46 procedural chapters → ~29 folios (87% of *Testamentum* expanded procedural content) |
